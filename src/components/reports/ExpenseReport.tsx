...
const ExpenseReport = ({ isReadOnly }: ExpenseReportProps) => {
  ...
  const fetchExpenseData = async () => {
    ...
    try {
      ...
      const expenses = await api.getPengeluaran();
      ...
      if (filterType === "month") {
        ...
        filteredData = expenses.filter(item => {
  ...
  const handleDownloadExcel = () => {
    try {
      ...
      if (filterType === "month") {
        ...
        fileName = `Laporan_Pengeluaran_${monthLabel}_${year}.xlsx`;
      } else {
        fileName = `Laporan_Pengeluaran_Tahun_${selectedYear}.xlsx`;
      }
  ...
  const handleDownloadPDF = () => {
    try {
      ...
      if (filterType === "month") {
        ...
        title = `Laporan Pengeluaran ${monthLabel}`;
      } else {
        title = `Laporan Pengeluaran Tahun ${selectedYear}`;
      }
      ...
      const tableRows = expenseData.map((expense, index) => [
        index + 1,
        formatDate(expense.tanggal),
        expense.keterangan,
        formatRupiah(expense.jumlah).replace("Rp", "").trim()
      ]);
      ...
      doc.setFontSize(10);
      doc.text(`Total Pengeluaran: ${formatRupiah(totalExpense)}`, 14, finalY + 10);
      ...
      if (filterType === "month") {
        const [year, month] = selectedMonth.split('-');
        const monthLabel = months.find(m => m.value === selectedMonth)?.label.split(' ')[0] || `Bulan-${month}`;
        fileName = `Laporan_Pengeluaran_${monthLabel}_${year}.pdf`;
      } else {
        fileName = `Laporan_Pengeluaran_Tahun_${selectedYear}.pdf`;
      }
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-xl flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <span>Laporan Pengeluaran Madrasah</span>
            ...
            <div className="flex flex-col space-y-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button onClick={fetchExpenseData} size="sm">
                  Tampilkan
                </Button>
              </div>
            </div>
                  ...
                  {expenseData.length > 0 ? (
                    expenseData.map((expense, index) => (
                      <TableRow key={expense.id}>
                        ...
                        <TableCell className="text-right">{formatRupiah(expense.jumlah)}</TableCell>
                      </TableRow>
                    ))
                  ) : (
                    <TableRow>
                      <TableCell colSpan={4} className="text-center py-10 text-gray-500">
                        {filterType === "month" 
                          ...
                          : "Tidak ada data pengeluaran pada tahun yang dipilih"}
                      </TableCell>
                    </TableRow>
                  )}
                </TableBody>
        ...
        <CardFooter className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pt-4 border-t">
          ...
          <div className="flex gap-2">
            <Button 
              ...
              disabled={loading || isReadOnly || expenseData.length === 0}
            >
